/**
 * Ícones do painel — SVG inline (sem dependência nova), padronizados no
 * estilo Heroicons outline (24x24, stroke 1.5, MIT license). `stroke="currentColor"`
 * faz a cor herdar do texto do elemento pai (`text-primary`/`text-base-content`/
 * etc já usados no painel), então acompanha a paleta automaticamente sem
 * precisar de classe extra. Pedido explícito do usuário (2026-08-20): nada
 * de emoji como ícone em nenhum lugar do painel — só esses SVGs.
 */
const ICON_PATHS: Record<string, string> = {
  ...
  folder: `<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-19.5 0v6a2.25 2.25 0 0 0 2.25 2.25h15a2.25 2.25 0 0 0 2.25-2.25v-6m-19.5 0v-1.5A2.25 2.25 0 0 1 4.5 9h4.586a1.5 1.5 0 0 1 1.06.44l1.415 1.414a1.5 1.5 0 0 0 1.06.44H19.5a2.25 2.25 0 0 1 2.25 2.25v1.5" />`,
  ...
  sparkles: `<path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.455-2.456L14.25 6l1.035-.259a3.375 3.375 0 0 0 2.456-2.455L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456Z" />`,
  ...
  "arrow-up-tray": `<path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 7.5m0 0L7.5 12m4.5-4.5v9" />`,
};

/** "hand-thumb-down" não é um ícone à parte — é o "hand-thumb-up" espelhado
 * via rotate-180 (evita depender de um segundo path complexo só pra isso). */
export function icon(name: string, className = "w-4 h-4 inline-block"): string {
  const isThumbDown = name === "hand-thumb-down";
  const path = ICON_PATHS[isThumbDown ? "hand-thumb-up" : name];
  if (!path) return "";
  const cls = isThumbDown ? `${className} rotate-180` : className;
  return `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="${cls}" aria-hidden="true">${path}</svg>`;
}
